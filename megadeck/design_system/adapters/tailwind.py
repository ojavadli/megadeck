"""Adapter: TailwindCSS color palettes → megadeck themes.

Source data
-----------
Colour values are the published Tailwind palette spec (MIT-licensed):
  https://tailwindcss.com/docs/customizing-colors
  https://github.com/tailwindlabs/tailwindcss/blob/main/packages/tailwindcss/src/utilities.ts

We hard-code the canonical 22 palettes × 11 shades. Combined with the 9
visual-style recipes in `themegen.py`, this gives 22 × 9 = 198 distinct
themes from a single import. Each one uses the palette's `500` shade as
the accent, `700` as accent_dk, `200` as accent_lt.
"""
...
# Tailwind v3 default palette (verbatim from tailwindcss source).
TAILWIND_PALETTES: Dict[str, Dict[int, str]] = {
...
def palette_from_tailwind(name: str) -> Palette:
    """Build a megadeck `Palette` from a Tailwind palette name."""
    if name not in TAILWIND_PALETTES:
        raise ValueError(f"Unknown Tailwind palette: {name!r}")
    p = TAILWIND_PALETTES[name]
    return Palette(
        name=f"tailwind-{name}",
        accent=p[500],
        accent_dk=p[700],
        accent_lt=p[200],
        bg_dark=p[950],
...
def generate_tailwind_themes(
    visual_styles: List[VisualStyle] = None,
) -> Iterator[Dict]:
    """Yield a theme dict for every (palette, style) combination."""
    if visual_styles is None:
        visual_styles = list(VISUAL_STYLES.keys())  # all 9 styles
    for palette_name in TAILWIND_PALETTES:
        palette = palette_from_tailwind(palette_name)
        is_neutral = palette_name in ("slate", "gray", "zinc", "neutral", "stone")
        for style in visual_styles:
            # Light mode by default; darker neutrals → "dark" mode
            mode = "dark" if (is_neutral and style in ("orbs", "corner-glow", "glass")) else "light"
            yield generate_theme(
                palette,
                visual_style=style,
                mode=mode,
                name=f"tw-{palette_name}-{style}",
                description=f"Tailwind {palette_name} ({style}).",
            )
